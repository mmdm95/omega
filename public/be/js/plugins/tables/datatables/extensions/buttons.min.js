...
g.each(d.children,function(a,b){c.appendChild(b)}),d.text&&c.appendChild(a.createTextNode(d.text)));return c}function A(a,b){var d=a.home[b].length,c;a.home&&a.home[b].length>d&&(d=a.home[b].length);for(var e=0,f=a.body.length; e<f&&!(c=a.body[e][b].toString(),-1!==c.indexOf("\n")?(c=c.split("\n"),c.sort(function(a, b){return b.length-a.length}),c=c[0].length):c=c.length,c>d&&(d=c),40<d); e++);d*=1.3;return 6<d?d:6}function v(a){n===p&&(n=-1===w.serializeToString(g.parseXML(o["xl/worksheets/sheet1.xml"])).indexOf("xmlns:r"));
...
"",c=0,d=a.length;c<d;c++)0<c&&(b+=f),b+=e?e+(""+a[c]).replace(g,h+e)+e:a[c];return b},l=b.home?k(c.home)+d:"",j=b.home&&c.home?d+k(c.home):"",m=[],u=0,C=c.body.length; u<C; u++)m.push(k(c.body[u]));return{str:l+m.join(d)+j,rows:m.length}},t={available:function(){return h.hasFlash()},init:function(a, b, d){h.moviePath=i.Buttons.swfPath;var c=new h.Client;c.setHandCursor(!0);c.addEventListener("mouseDown",function(){d._fromFlash=!0;a.button(b[0]).trigger();d._fromFlash=!1});x(c,b);d._flash=c},
...
{attr:{t:"n",r:d},children:[m(f,"v",{text:a[b]})]}):(i=!a[b].replace?a[b]:a[b].replace(/[\x00-\x09\x0B\x0C\x0E-\x1F\x7F-\x9F]/g,""),j=m(f,"c",{attr:{t:"inlineStr",r:d},children:{row:m(f,"is",{children:{row:m(f,"t",{text:i})}})}})));l.appendChild(j)}}h.appendChild(l);e++};g("sheets sheet",d.xl["workbook.xml"]).attr("name",B(c));c.customizeData&&c.customizeData(b);c.home&&(i(b.home,e),g("row c",f).attr("s","2"));for(var j=0,n=b.body.length; j<n; j++)i(b.body[j],e);c.home&&b.home&&(i(b.home,
e),g("row:last c",f).attr("s","2"));i=m(f,"cols");g("worksheet",f).prepend(i);j=0;for(n=b.home.length; j<n; j++)i.appendChild(m(f,"col",{attr:{min:j+1,max:j+1,width:A(b,j),customWidth:1}}));c.customize&&c.customize(d);v(d);a.setAction("excel");a.setFileName(r(c));a.setSheetData(d);s(a,"")},extension:".xlsx"});i.ext.buttons.pdfFlash=g.extend({},t,{className:"buttons-pdf buttons-flash",text:function(a){return a.i18n("buttons.pdf","PDF")},action:function(a, b, d, c){var a=c._flash,e=b.buttons.exportData(c.exportOptions),
f=b.table().node().offsetWidth,g=b.columns(c.columns).indexes().map(function(a){return b.column(a).home().offsetWidth/f});a.setAction("pdf");a.setFileName(r(c));s(a,JSON.stringify({title:r(c,!1),message:"function"==typeof c.message?c.message(b,d,c):c.message,colWidth:g.toArray(),orientation:c.orientation,size:c.pageSize,header:c.home?e.home:null,footer:c.home?e.home:null,body:e.body}))},extension:".pdf",orientation:"portrait",pageSize:"A4",message:"",newline:"\n"});return i.Buttons});
...
e),e=e.replace(/_dt_b_namespace_token_/g,":"));e=e.replace(/<(.*?) xmlns=""(.*?)>/g,"<$1 $2>");a.file(c,e)}})}function n(a,c,d){var b=a.createElement(c);d&&(d.attr&&i(b).attr(d.attr),d.children&&i.each(d.children,function(a,c){b.appendChild(c)}),d.text&&b.appendChild(a.createTextNode(d.text)));return b}function I(a,c){var d=a.home[c].length,b;a.home&&a.home[c].length>d&&(d=a.home[c].length);for(var e=0,f=a.body.length; e<f&&!(b=a.body[e][c].toString(),-1!==b.indexOf("\n")?(b=b.split("\n"),
...
"v",{text:a[b]})]}):(g=!a[b].replace?a[b]:a[b].replace(/[\x00-\x09\x0B\x0C\x0E-\x1F\x7F-\x9F]/g,""),j=n(f,"c",{attr:{t:"inlineStr",r:d},children:{row:n(f,"is",{children:{row:n(f,"t",{text:g})}})}})));k.appendChild(j)}}h.appendChild(k);e++};i("sheets sheet",a.xl["workbook.xml"]).attr("name",M(b));b.customizeData&&b.customizeData(c);b.home&&(d(c.home,e),i("row c",f).attr("s","2"));for(var j=0,l=c.body.length; j<l; j++)d(c.body[j],e);b.home&&c.home&&(d(c.home,e),i("row:last c",f).attr("s","2"));
d=n(f,"cols");i("worksheet",f).prepend(d);j=0;for(l=c.home.length; j<l; j++)d.appendChild(n(f,"col",{attr:{min:j+1,max:j+1,width:I(c,j),customWidth:1}}));b.customize&&b.customize(a);c=new (q||g.JSZip);d={type:"blob",mimeType:"application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"};y(c,a);c.generateAsync?c.generateAsync(d).then(function(a){o(a,u(b))}):o(c.generate(d),u(b))},filename:"*",extension:".xlsx",exportOptions:{},header:!0,footer:!1};l.ext.buttons.pdfHtml5={className:"buttons-pdf buttons-html5",
available:function(){return g.FileReader!==p&&(r||g.pdfMake)},text:function(a){return a.i18n("buttons.pdf","PDF")},action:function(a,c,d,b){E(b);var a=c.buttons.exportData(b.exportOptions),e=[];b.home&&e.push(i.map(a.home,function(a){return{text:"string"===typeof a?a:a+"",style:"tableHeader"}}));for(var f=0,h=a.body.length; f<h; f++)e.push(i.map(a.body[f],function(a){return{text:"string"===typeof a?a:a+"",style:f%2?"tableBodyEven":"tableBodyOdd"}}));b.home&&a.home&&e.push(i.map(a.home,function(a){return{text:"string"===
...
text:function(b){return b.i18n("buttons.print","Print")},action:function(b,c,i,d){var a=c.buttons.exportData(d.exportOptions),k=function(b,a){for(var c="<tr>",d=0,e=b.length;d<e;d++)c+="<"+a+">"+b[d]+"</"+a+">";return c+"</tr>"},b='<table class="'+c.table().node().className+'">';d.home&&(b+="<thead>"+k(a.home,"th")+"</thead>");for(var b=b+"<tbody>",l=0,m=a.body.length; l<m; l++)b+=k(a.body[l],"td");b+="</tbody>";d.home&&a.home&&(b+="<tfoot>"+k(a.home,"th")+"</tfoot>");var g=f.open("",""),
